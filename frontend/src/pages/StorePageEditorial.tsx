...
import { fetchProductsByIds, fetchProducts } from "../lib/catalog";
import { SHOP_CATEGORY_CHIPS, SHOP_USE_CASES, STORE_NAV_CATEGORIES } from "../lib/shop";
import {
  ...
  return (
    ...
    >
      <Box
        ...
        alt={lang === "en" ? "Three engraved metal bracelets" : "שלושה צמידי מתכת עם המסר"}
        sx={{
          position: "absolute",
          inset: 0,
          width: "100%",
      ...
      >
        <Box sx={{ maxWidth: { xs: 520, md: 650 } }}>
          <Typography
            ...
            sx={{
              ...displayHeadingSx,
              fontSize: { xs: "clamp(3.1rem, 15vw, 5rem)", md: "clamp(5rem, 8vw, 7.5rem)" },
              textWrap: "balance",
            }}
          >
            {copy.heroTitle}
...
function CategorySection() {
  ...
  return (
    <Box component="section" sx={{ py: { xs: 5, md: 8 }, bgcolor: "#fffdf8", overflow: "hidden" }}>
      <Box sx={{ maxWidth: STORE_MAX_WIDTH, mx: "auto", px: { xs: 2, md: 3.5 } }}>
        <Stack
          direction={{ xs: "column", sm: "row" }}
          sx={{ justifyContent: "space-between", alignItems: { sm: "flex-end" }, mb: { xs: 2.5, md: 3.5 }, gap: 2 }}
        >
          <Box>
            <Typography component="h2" sx={{ ...displayHeadingSx, fontSize: { xs: 30, md: 42 } }}>
...
function FeaturedEdit({
  ...
  loading: boolean;
  products: Map<number, Awaited<ReturnType<typeof fetchProductsByIds>>["items"][number]>;
}) {
  const { lang, loc } = useLocale();
  const copy = STORE_COPY[lang];
      ...
      >
        ...
        <Box>
          <Box sx={{ maxWidth: 530, mb: { xs: 4, md: 6 } }}>
            ...
            <Typography component="h2" sx={{ ...displayHeadingSx, fontSize: { xs: 40, md: 60 } }}>
              {copy.featuredTitle}
            </Typography>
            <Typography sx={{ mt: 2, fontSize: { xs: 15, md: 17 }, lineHeight: 1.7, color: "rgba(17,17,17,.7)" }}>
              {copy.featuredBody}
            </Typography>
          </Box>
          {loading ? (
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: { xs: 1.5, md: 3 } }}>
              {entries.map((entry) => {
                ...
                return (
                  <ProductCard
                    ...
                    imageOverride={{
                      src: entry.src,
                      hoverSrc: entry.productId === 42537 ? entry.hoverSrc : undefined,
                      alt: entry.alt[lang],
                    }}
                  />
                );
  ...
  return (
    <Box component="section" sx={{ bgcolor: "#fffdf8", py: { xs: 7, md: 12 } }}>
      <Box sx={{ maxWidth: STORE_MAX_WIDTH, mx: "auto", px: { xs: 2, md: 3.5 } }}>
        <Stack
          direction={{ xs: "column", sm: "row" }}
          sx={{ justifyContent: "space-between", alignItems: { sm: "flex-end" }, gap: 2, mb: { xs: 3.5, md: 5 } }}
        >
          <Box>
            <Typography sx={sectionLabelSx}>{copy.popularEyebrow}</Typography>
            <Typography component="h2" sx={{ ...displayHeadingSx, fontSize: { xs: 38, md: 56 } }}>
              {copy.popularTitle}
            </Typography>
          </Box>
          <Link
            ...
            {entries.map((entry) => {
              ...
              return (
                <ProductCard
                  ...
                  imageOverride={{
                    src: entry.src,
                    hoverSrc: entry.productId === 6065 || entry.productId === 6080 ? entry.hoverSrc : undefined,
                    alt: entry.alt[lang],
                  }}
                />
              );
    ...
    <Box component="section" sx={{ bgcolor: "#123e3e", color: "#fff" }}>
      ...
      >
        <Box
          ...
          sx={{ width: "100%", height: "100%", minHeight: { xs: 450, md: 700 }, objectFit: "cover", display: "block" }}
        />
        <Stack sx={{ px: { xs: 3, md: 9 }, py: { xs: 7, md: 10 }, justifyContent: "center", maxWidth: 700 }}>
          <Typography sx={{ ...sectionLabelSx, color: "rgba(255,255,255,.72)" }}>{copy.storyEyebrow}</Typography>
          <Typography component="h2" sx={{ ...displayHeadingSx, fontSize: { xs: 48, md: 78 }, whiteSpace: "pre-line" }}>
            {copy.storyTitle}
          </Typography>
          <Typography sx={{ mt: 2.5, maxWidth: 500, fontSize: { xs: 16, md: 18 }, lineHeight: 1.75, color: "rgba(255,255,255,.78)" }}>
            {copy.storyBody}
          </Typography>
          <Button
            component={RouterLink}
...
function CollectionPage({ category }: { category: string }) {
  ...
  const query = useQuery({
    queryKey: ["products", "collection", page, category],
    queryFn: () => {
      const search = new URLSearchParams({ page: String(page), per_page: "24" });
  ...
  return (
    <>
      <Box ref={topRef} sx={{ bgcolor: "#f8f6f1", pt: { xs: 6, md: 10 }, pb: { xs: 7, md: 12 } }}>
        <Box sx={{ maxWidth: STORE_MAX_WIDTH, mx: "auto", px: { xs: 2, md: 3.5 } }}>
          <Box sx={{ mb: { xs: 4, md: 6 } }}>
            <Typography component="h1" sx={{ ...displayHeadingSx, fontSize: { xs: 48, md: 76 } }}>
              {title}
            </Typography>
            {query.data && (
          ...
          {query.isError && <ErrorState message={lang === "en" ? "The collection could not be loaded." : "לא ניתן לטעון את הקולקציה."} onRetry={() => void query.refetch()} />}
          {query.data?.items.length === 0 && <EmptyState>{copy.noResults}</EmptyState>}
...
export function StorePageEditorial() {
  ...
  const homepageProducts = useQuery({
    queryKey: ["products", "homepage", "editorial-store", STORE_HOMEPAGE_PRODUCT_IDS],
    queryFn: async () => {
  ...
  return (
    <>
      <StoreHero />
      <CategorySection />
      <FeaturedEdit loading={homepageProducts.isLoading} products={homepageProducts.data ?? new Map()} />
      <PopularSection loading={homepageProducts.isLoading} products={homepageProducts.data ?? new Map()} />
      <StorySection />
    </>
  );
}
